...
struct PQueue {
    ...
    PQueue() : front(nullptr), last(nullptr) {}
};
...
void Enqueue(PQueue& Q, int value) {
    ...
    if (Q.last) {
        ...
        Q.front = newNode; // Если очередь была пустой
    }
    Q.last = newNode;
}
...
int main() {
    ...
    if (Q.front != nullptr) {
        cout << "Адрес начала очереди: " << Q.front << ", Значение: " << Q.front->data << endl;
    } else {
        cout << "Очередь пуста, адрес начала: nullptr" << endl;
    }

    if (Q.last != nullptr) {
        cout << "Адрес конца очереди: " << Q.last << ", Значение: " << Q.last->data << endl;
    } else {
